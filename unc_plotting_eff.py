...
# ------------------------------------------------------------------
# Plot one efficiency uncertainty file
# ------------------------------------------------------------------
def plot_quantity(quantity, channel, era):
    input_dir  = os.path.join("rootfiles",    "eff_uncertainties",  f"{channel}_{era}")
    output_dir = os.path.join("plots_tagging", "eff_uncertainties", f"{channel}_{era}")
    ...
    label = "MC" if quantity.startswith("mc") else "Data"
    title = (
        f"{label} efficiency unc - {channel} {era}"
        f";FatJet p_{{T}} [GeV];Efficiency (varied - nominal)"
    )

    y_min = Y_MIN_DATA_EFF if quantity.startswith("data") else Y_MIN_MC_EFF
    y_max = Y_MAX_DATA_EFF if quantity.startswith("data") else Y_MAX_MC_EFF
